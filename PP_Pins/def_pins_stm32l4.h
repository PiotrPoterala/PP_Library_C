#ifndef _DEF_PINS_STM32L4_H
	#define _DEF_PINS_STM32L4_H
...
//input pull-up/pull-down
	...
	#define PIN_CLR(PORT, PIN) (PORT->BRR=PIN)
	
	#define PIN_IS_SET(PORT, PIN) (((PORT->IDR & PIN)>0)?1:0)	
	
	
	#define IO_SET_REG1(REG, PIN, SETS) ((REG & ~(0x1<<(PIN))) | ((SETS)<<(PIN)))
	#define IO_SET_REG2(REG, PIN, SETS) (REG & ~(0x3u<<(PIN*2))) | ((SETS)<<(PIN*2))
	
  #define IN_SET_REG(REG, PIN, SET1) (REG->MODER=IO_SET_REG2(REG->MODER, PIN, IO_IN), REG->PUPDR=IO_SET_REG2(REG->PUPDR, PIN, SET1))
	#define OUT_SET_REG_WITH_TYPE(REG, PIN, TYPE, SPEED) (REG->MODER=IO_SET_REG2(REG->MODER, PIN, IO_OUT), REG->OTYPER=IO_SET_REG1(REG->OTYPER, PIN, TYPE), REG->OSPEEDR=IO_SET_REG2(REG->OSPEEDR, PIN, SPEED))
  #define OUT_SET_REG(REG, PIN, SPEED) (REG->MODER=IO_SET_REG2(REG->MODER, PIN, IO_OUT), REG->OTYPER&=~(1<<PIN), REG->OSPEEDR=IO_SET_REG2(REG->OSPEEDR, PIN, SPEED))
